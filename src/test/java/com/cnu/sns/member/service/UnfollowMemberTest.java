...
@SpringBootTest
@Transactional
public class UnfollowMemberTest {
    @Autowired
    ...
    public void beforeAll() {
        ...
        relationRepository.save(new Relation(mem1, mem2));
    }

    @Test
    public void bothPresent() {
        int result = memberService.unfollowMember("testid1","testid2");
        Assertions.assertEquals(1, result);

        Assertions.assertFalse(relationRepository.findById(new RelationId("testid1", "testid2")).isPresent());
    }
    @Test
    public void followerAbsent() {
        int result = memberService.unfollowMember("absentId", "testid2");
        Assertions.assertEquals(0, result);
    }
    @Test
    public void followeeAbsent() {
        int result = memberService.unfollowMember("testid1", "absentId");
        Assertions.assertEquals(0, result);
    }
}
